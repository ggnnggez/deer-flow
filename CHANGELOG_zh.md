...
#### 扩展与插件
- **示例：** 上下文留存页面升级为控制台。**任务列表**页列出每个已记录的任务及其步数、
  attempt 数、压缩次数、清单完整度和峰值请求（按类型、结果、时间范围、有无压缩、清单是否
  完整筛选；按会话分组或平铺；可排序、分页），粘贴完整任务 id 直接打开看板。**健康度**页
  展示扩展自身的记录状态：队列、已接收 / 已写入 / 已丢弃 / 写入失败计数、每分钟写入吞吐、
  各表行数、清单完整度与压缩定位质量、带处理建议的诊断，以及只读的插件配置回显。两条新的
  管理员路由提供数据：`GET /api/context-residency/tasks` 与
  `GET /api/context-residency/health`。每个 attempt 现在记录模型的**上下文窗口**（模型配置
  的 `context_window`，否则插件的 `context_windows` / `default_context_window`，从不猜测），
  看板的组成占比条以该窗口为 100%：每一道显示其占窗口的绝对百分比，其余为剩余空间，超出
  窗口的请求会被标出；未记录窗口时占比条以本次请求为 100% 并如实说明。([#TBD])
