...
| `kept_content_hashes`     | `tuple[str, ...]` (default `()`, 0.2.5) |
| `task_id`                 | `str \| None` (default `None`, 0.2.5) |
| `run_id`                  | `str \| None` (default `None`, 0.2.5) |
| `thread_id`               | `str \| None` (default `None`, 0.2.5) |
| `emitted_at`              | `str \| None` (ISO-8601 UTC, default `None`, 0.2.5) |

Hashes are `canonical_hash(message.content)` of the content passed directly, never a stringified copy. To match a message to an event, hash its `content` the same way. The identity fields are copied from the task's seeded `TaskInfo` and stay `None` when there is none.
...
| `MESSAGE_SUMMARY_CONTENT_HASH_KEY` | `"deerflow_summary_content_hash"` (0.2.6) |
| `PROVENANCE_KEYS`                | `frozenset` of the four keys. The host treats them as server-owned and strips caller-supplied values from untrusted input |
...
Frozen dataclass: `content_kind: str`, `producer_kind: str`, `producer_entity_id: str | None = None`, `summary_content_hash: str | None = None` (0.2.6). The last is set on a message that renders a compaction summary and equals the `CompactionEvent.output_content_hash` of the compaction that produced it — the identity the host records at compaction, never a rehash of the rendering.

### `provenance_kwargs(content_kind, producer_kind, *, producer_entity_id=None, summary_content_hash=None) -> dict[str, str]`

The `additional_kwargs` fragment to merge into a message you produce. `producer_entity_id` and `summary_content_hash` are omitted when `None`.

### `read_provenance(message: object) -> MessageProvenance | None`

Read a stamp from `message.additional_kwargs`. Returns `None` when either required key is missing or not a string; a malformed optional field reads as `None` without discarding the stamp.
...
| 0.2.4 | [#5842](https://github.com/bytedance/deer-flow/pull/5842) | Plugin resource authorization: `require_plugin_management`, `arequire_plugin_management`, `EXTENSION_PLUGIN_AUTHZ_RESOLVER_KEY`, `EXTENSION_PLUGIN_AUTHZ_RESOLVER_ASYNC_KEY` |
| 0.2.5 | [#TBD](https://github.com/bytedance/deer-flow/pull/TBD) | `CompactionEvent.kept_content_hashes`, `task_id`, `run_id`, `thread_id`, `emitted_at`; the host seeds every task store with its `TaskInfo` |
| 0.2.6 | [#TBD](https://github.com/bytedance/deer-flow/pull/TBD) | `MESSAGE_SUMMARY_CONTENT_HASH_KEY`, `MessageProvenance.summary_content_hash`, `provenance_kwargs(summary_content_hash=)`; summarization records `summary_content_hash` in state and the durable-context data block declares the summary it carries |
