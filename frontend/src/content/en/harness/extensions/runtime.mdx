...
The host seeds each task store with the scope's `TaskInfo` before any hook runs, so a hook that only holds the runtime — a contributed middleware, a compaction observer reading the event's identity — can learn which task it is in with `task_store.get(TaskInfo)`. Treat that entry as read-only; it is the same value `on_task_start` receives. A subagent executed without a parent run id has no identity to seed, and the entry is absent.
