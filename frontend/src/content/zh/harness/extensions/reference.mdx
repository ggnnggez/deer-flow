...
| `kept_content_hashes`     | `tuple[str, ...]`（默认 `()`，0.2.5） |
| `task_id`                 | `str \| None`（默认 `None`，0.2.5） |
| `run_id`                  | `str \| None`（默认 `None`，0.2.5） |
| `thread_id`               | `str \| None`（默认 `None`，0.2.5） |
| `emitted_at`              | `str \| None`（ISO-8601 UTC，默认 `None`，0.2.5） |

哈希是对直接传入的内容计算的 `canonical_hash(message.content)`，从不是字符串化后的副本。要把某条消息与事件对应起来，请用同样的方式哈希它的 `content`。身份字段取自任务已播种的 `TaskInfo`，没有时保持 `None`。
...
| `PROVENANCE_KEYS`                | 这四个键组成的 `frozenset`。宿主把它们视为服务端所有，并从不可信输入中剥离调用方提供的值 |
...
冻结的 dataclass：`content_kind: str`、`producer_kind: str`、`producer_entity_id: str | None = None`、`summary_content_hash: str | None = None`（0.2.6）。最后一项只出现在渲染压缩摘要的消息上，等于产出该摘要的那次压缩的 `CompactionEvent.output_content_hash`——是宿主在压缩时记录的身份，从不是对渲染结果的重新哈希。

### `provenance_kwargs(content_kind, producer_kind, *, producer_entity_id=None, summary_content_hash=None) -> dict[str, str]`

要合并进你所生成消息的 `additional_kwargs` 片段。`producer_entity_id` 与 `summary_content_hash` 为 `None` 时省略。

### `read_provenance(message: object) -> MessageProvenance | None`

从 `message.additional_kwargs` 读取标记。任一必需键缺失或不是字符串时返回 `None`；可选字段格式不对时读作 `None`，不会丢弃整个标记。
...
| 0.2.4 | [#5842](https://github.com/bytedance/deer-flow/pull/5842) | 插件资源授权：`require_plugin_management`、`arequire_plugin_management`、`EXTENSION_PLUGIN_AUTHZ_RESOLVER_KEY`、`EXTENSION_PLUGIN_AUTHZ_RESOLVER_ASYNC_KEY` |
| 0.2.5 | [#TBD](https://github.com/bytedance/deer-flow/pull/TBD) | `CompactionEvent.kept_content_hashes`、`task_id`、`run_id`、`thread_id`、`emitted_at`；宿主用各任务的 `TaskInfo` 播种任务存储 |
| 0.2.6 | [#TBD](https://github.com/bytedance/deer-flow/pull/TBD) | `MESSAGE_SUMMARY_CONTENT_HASH_KEY`、`MessageProvenance.summary_content_hash`、`provenance_kwargs(summary_content_hash=)`；摘要压缩把 `summary_content_hash` 记入状态，durable-context 数据块声明它承载的摘要 |
