...
宿主会在任何钩子运行之前，用该作用域的 `TaskInfo` 播种每个任务存储，所以只持有运行时的钩子——贡献的中间件、读取事件身份的压缩观察者——可以用 `task_store.get(TaskInfo)` 得知自己身处哪个任务。把这个条目当作只读；它与 `on_task_start` 收到的值相同。没有父 run id 的子代理执行没有可播种的身份，该条目不存在。
