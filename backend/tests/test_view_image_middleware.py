...
class TestRequestScopedObservationMetadata:
    ...
    def test_internal_checkpoint_marker_is_removed_without_ansich(self):
        ...
        message = HumanMessage(
            ...
            additional_kwargs={"hide_from_ui": True, _IMAGE_CONTEXT_MESSAGE_MARKER_KEY: True},
        )
        captured = {}
        ...
        assert _IMAGE_CONTEXT_MESSAGE_MARKER_KEY not in captured["message"].additional_kwargs
    ...
    def test_ansich_markers_exist_only_on_the_model_request_copy(self):
        ...
        assert _IMAGE_CONTEXT_MESSAGE_MARKER_KEY not in request_message.additional_kwargs
        ...
        assert _IMAGE_CONTEXT_MESSAGE_MARKER_KEY in persisted.additional_kwargs
...
class TestAfterModel:
    ...
    def test_graph_preserves_normalized_client_message_with_reserved_prefix(self, tmp_path):
        ...
        # The injected transient message reaches the model. Detect it by its
        # reserved ID prefix: wrap_model_call strips the neutral marker from the
        # model-request copy (it is checkpoint metadata, not provider payload).
        assert any(isinstance(message, HumanMessage) and message.id != client_id and message.id and message.id.startswith("view-image-context:") for message in capture.messages)
        assert all(message.additional_kwargs.get(_IMAGE_CONTEXT_MESSAGE_MARKER_KEY) is not True for message in capture.messages)
