...
def test_public_make_lead_agent_does_not_take_mode_from_runtime_context(monkeypatch):
    ...
    def _capture(config, *, app_config):
        ...
        captured["app_config"] = app_config
        # ``make_lead_agent`` unwraps ``.graph`` from the assembly result.
        return SimpleNamespace(graph=object())

    # The public path now assembles via ``_assemble_lead_agent`` (returning a
    # LeadAgentAssembly); ``_make_lead_agent`` stays the internal graph-only ABI.
