...
class MemoryManager(BaseModel):
    ...
    def add_nowait(
        ...
        *,
        ...
        priority).
        """
        self.add(thread_id, messages, agent_name=agent_name, user_id=user_id, observability_context=observability_context)
...
def _collect_host_hooks() -> dict[str, Any]:
    ...
    *consumer* that decides which hooks to use (so adding a backend does not
    ...
    return {
        ...
        # Ansich system-operation probe: wraps the memory-update model.invoke so
        # the opaque observability context enqueued with the conversation is
        # observed on the debounce worker thread (DeerMem consumes; noop ignores).
        "model_invoke_callback": _host_default_memory_model_invoke,
    }
