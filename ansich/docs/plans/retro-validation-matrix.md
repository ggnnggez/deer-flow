...
**1. 预测命中率：可计分的 5 条里命中 4 条，1 条偏保守。** A/D/E/F 四条预测与实际完全一致；C 比预测好（我预测 effects 视图可能缺失或退化，实际它完整且诚实）；B 事前登记的就是"不确定"，无法计分，实测为强通过。**但高命中率本身是个警告信号，不是好消息**——这些预测是在读完实现代码之后做的，所以它主要证明"我读懂了代码"，而不是"设计经受住了考验"。六条全绿说明这套矩阵的对抗性不足。真正产生新信息的只有两条：B（事前确实不知道）和 F（产出了一个可执行的缺口）。
