...
| ---- | ---- | ---- | -------- | ------ |
| M1 | `wall_time_ms/local` 由 `_project_heartbeat` 与 `_project_usage` 双写者共同拥有,职责重叠 | ✅ 已修复 | 2026-08-19 | `459c170e` |
| M2 | heartbeat 每 tick 落一条持久 wall_time contribution,叠加 recompute-per-insert 的 summary 刷新,长任务写/读放大随 tick 平方增长 | ✅ 已修复(方向 ①) | 2026-08-19 | `7c8edb26` / `2443fc99` |
...
- 状态:✅ 已修复(方向 ① / 裁决 HR1,2026-08-19,`7c8edb26`;并发加锁与迁移重放钉子随 `2443fc99` 补齐)。wall_time 现按 max 型高水位通道存储:heartbeat 来源的 `wall_time_ms` contribution 对每个 `(aggregate_task_id, source_task_id)` 只保留一行,新 tick 仅在抬高水位时替换该行,不再每 tick 追加。方向 ②(sum 维度的通用增量刷新)按 HR1 留待 Phase 11。
  ...
  - **并发(Phase 11 前置)**:`_upsert_high_water_contribution` 是 read-modify-write,被替换掉的 `ON CONFLICT DO NOTHING` 则天然原子,因此按 `_recompute_release_quality_stats`(sql.py,Phase 10 质量单元格)的先例,在**读取之前**用 `.with_for_update(of=AnsichUsageContributionRow)` 锁住该行:多 worker 下同一 Task 的每个 tick 是独立投影 job(`_claim_projection_job` 用 `skip_locked`),ancestry fan-out 还让兄弟 Task 争用同一祖先行,READ COMMITTED 下未加锁的读可能在对端提交更高水位前读到旧值,再 delete+insert 写回更低值(lost update);读后再锁窗口不变。`of=` 只锁 contribution 行,不锁 join 进来的不可变 observation 行;SQLite 单写者,该子句被忽略。`_backfill_spawn_usage` 的读**刻意不加锁**并在 docstring 说明理由:它读后代自身行、只写祖先行(ancestry 无环且 self-free,两个行集不相交),不构成对同一行的 read-modify-write;其所有写入仍走 `_store_usage_contribution`,高水位写走加锁的 max 路径,max 单调,故并发下最坏只是拷到的水位落后一个 tick、由下一个 tick 的 fan-out 抬平,永远不会压低或高报。真正的并发测试在本仓库不可行(与 Phase 10 质量单元格同一姿态),回归保护由注释与批次终审承担。
...
- 迁移:`0024_ansich_wall_time_watermarks`(chains from `0023_ansich_evaluations`)。纯数据迁移、无 schema 变更:按与投影器同一套 `(delta, as_of, source_obs_id)` 全序,删除被严格支配的 heartbeat 来源 wall_time 行,每个 `(aggregate, source)` 只留水位行;终态 `budget.consumed` 行与所有 sum 型维度不动。因所有消费端都以 max 归约同一 `(aggregate, source)` 的 `delta`,删除非最大行不改变 `value`(**无条件成立**),故 `ansich_task_usage` summary 刻意不重写。`as_of` / `complete_through_ingest_seq` 的保持有一个**前提**:同一 source 的 `occurred_at` 随 `elapsed_ms` 非降(心跳探针单调时钟与墙钟同向)。若发生 NTP 回拨,被按 `delta` 判为支配的行可能携带更大的 `as_of`;此时"不重写 summary"恰好是安全的:迁移后 summary 保留折叠前(全历史推出)的 `as_of`/watermark,折叠本身不会压低它,直到该 aggregate 的下一个 tick 或终态 contribution 触发 `_refresh_usage_summary`,从存活行重算——最坏把 `as_of` 回退一个心跳间隔,`value` 不受影响。迁移库与从观察重放的库在正常前提下收敛到同一 contribution 行集与同一 summary,由 `test_wall_time_watermark_migration_matches_a_replayed_projection` 锁定(该用例在单调 fixture 下天然为绿,其判别力用变异测试验证:把支配比较反向后立即变红)。upgrade 以 inspector 守卫建表缺失并可重复执行(0016/0023 先例);downgrade 结构上为 no-op——被删的是可重建的投影产物而非源事实,heartbeat 观察原样保留,`rebuild_projections()` 会按当时运行的代码版本重新生成(与 0019 downgrade 先删派生行的处理同源)。
