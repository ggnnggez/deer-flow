...
- 状态:✅ 已于 2026-08-19 修复(commit `a1bcf523`),按裁决 HR3 取「保守过期 + 请求驱动接管」方向,不做启动期扫描。
  ...
  - **恢复的边界(登记,不改本条状态)**:接管只对「Run 还活着」的孤儿生效——`_run_operator_action` 的「当前状态不支持该操作」409(`backend/app/gateway/routers/ansich.py:806`,即 `target.control_value != "running"` 或 Run 已终态)排在 `begin_operator_action` **之前**,所以 Task 已经离开 `running` 之后,同 key 重试连 SQL 层的过期判定都到不了,HR3 的恢复不会触发,那行孤儿 `requested` 会一直留在行表里(审计流本身不受影响)。
