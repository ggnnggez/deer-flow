...
- 状态:✅ 已修复(2026-08-19,本次变更待提交)。`_assess_scope_safety_at` 现在先用 watermark 区间 `(上次成功评估的 watermark, 本次 claim 的 watermark]` 反查本轮新增证据涉及的 `tool_call_id` 集合,只对该集合调用 `assess_scope_safety`/`_persist_assessment`/`_reconcile_alerts_for_assessment`;区间为空(例如终态 `task.completed` 触发的 job 本身不携带 Scope 证据)直接返回空结果。被选中的 tool_call 仍读取它自己 `<= watermark` 的**全量**证据,领域函数 `assess_scope_safety` 一行未改。**残留项(已知、已评估)**:该"取全量证据"的读取仍是一条对任务 Scope observation 的索引区间扫描(单条语句、O(N) 行,但只对命中集合做 pydantic 校验);无法进一步收窄,因为 `ansich_observations.subject_id` 存的是 snapshot_id/effect_id 而非 `tool_call_id`,从 tool_call 到原始证据没有索引路径,而改从 `ansich_tool_call_authorizations`/`ansich_tool_effects` 投影取证据会把 assessor 的证据来源从原始 Observation 日志换成投影状态——本次不做该语义变更。M2 诊断点名的 O(N²) 来源(逐 tool_call 重判 + `_persist_assessment`/`_reconcile_alerts_for_assessment` 往返)已消除。
- **区间下界的来源(三个候选的取舍)**:采用**新增持久表 `ansich_assessor_watermarks`**(migration `0025_ansich_assessor_watermarks`,主键 `(subject_id, assessor_name, assessor_version)`),在**与结论同一个事务**内推进。理由:
  - 候选(a)「从 `ansich_scope_conclusions` 行反推已评估位置」不可行——该行只有 `assertion_id`/`tool_call_id`/`conclusion_kind`,不存 `evaluated_obs_id`/`ingest_seq`;退而求其次用 assertion 的 evidence 反查 `ingest_seq` 也不成立:**没有违规结论的干净 tool_call 同样会写 4 条 `cleared` assertion,但一个从未产生任何 Scope 证据的任务其下界会永远停在 0**,且该路径只反映"写出去的结论",无法表达"已评估但无事发生"的覆盖范围。
  - 候选(b)「`AnsichAssessorJobRow` 中 `status='completed'` 的最大 `evidence_watermark`」**是错的,且会打破现有回归测试**:`_claim_assessor_job` 在 claim 事务里就把同组较低 watermark 的 job 置为 `completed`(此时评估还没跑),一旦这次评估失败/进程崩溃回滚,这些"已完成"的 job 对应的证据实际从未被评估,重试时的区间下界会把它们排除,证据被永久跳过。现有用例 `test_scope_safety_waits_for_subject_entity_then_self_heals` 正是这个形状:`authorization.evaluated` 的 job 在 claim 时被吸收成 `completed`,随后评估因 ToolCall Entity 未就绪抛 `_ProjectionDependencyPending` 回滚——若用(b),自愈后的区间将不含那条 authorization 证据,`scope_safety:policy_denial` 结论不会产生。**这一条经实测验证**:把下界临时换成(b)后,该用例即刻以 `assert belief is not None` → `None is not None` 失败,换回后恢复通过。
  - 候选(c)专用 watermark 列:此前不存在(`models.py` 里只有 assessor job 的 `evidence_watermark` 与 TaskSummary/ActiveTask 的 `projection_watermark`,语义都不是"assessor 已成功评估到哪"),故本次新增。
  - 该表满足 fact 2 的两个约束:**crash-safe**——写入位于评估自身的事务内,失败一起回滚,重试重新打开同一区间;**replay-safe**——`rebuild_projections()` 的删除列表里与 `AnsichScopeConclusionRow`/assertion 一同删除,重放后第一次评估必然走 cold-start 全量扫描,重建出完全相同的结论。
- **区间下界还要取 `min(mark, 本次 claim 组内最低 watermark - 1)`**:`_claim_assessor_job` 现在额外返回被吸收组的最低 watermark。若某条 Scope observation 的投影晚于更高 watermark 的兄弟落地,它的 assessor job 会带着**低于** mark 的 watermark 出现;只用 mark 会得到空区间而把它漏掉。取 min 后,区间恒覆盖"本次结算的所有 job 各自对应的 observation"∪"上次成功评估之后的全部新增证据"。
- **告警生命周期(核实结论)**:跳过未变更 tool_call 的 `_reconcile_alerts_for_assessment` 不影响任何告警生命周期。scope-safety 告警的 `subject_id` 就是 `tool_call_id`,而 `_resolve_terminal_alerts`/`_reconcile_alerts_for_assessment` 的终态短路都按 **task_id** 过滤 `AnsichAlertRow.subject_id`,因此这类告警本来就不参与终态批量 resolve;它们的 open/confirm/resolve 只由该 tool_call 自己的结论 `present`↔`cleared` 翻转驱动,而结论翻转必然伴随该 tool_call 的新证据、必然落在区间内。唯一的行为差异是:未变更结论对应的告警 `as_of` 不再被每个评估 tick 刷成"处理时间"——这与 `as_of` 应反映证据而非处理时刻的语义一致,且 `test_sql_alerts.py` 未覆盖 scope-safety 告警(全文件零命中),现有告警用例未做任何修改即保持绿色。
- 回归覆盖(均在 `tests/ansich/test_sql_safety.py`):`test_scope_safety_reassessment_work_does_not_grow_with_tool_call_count`(性能护栏,统计每轮 `assess_scope_safety` 调用数与 `ansich_scope_conclusions` 语句数,修复前为 `[1,2,3,4,5]`,修复后恒为 `[1,1,1,1,1]`)、`test_late_scope_evidence_reassesses_only_its_own_tool_call`(晚到证据只重估自身;已收敛 tool_call 的 assertion 条数/conclusion 行数/被选中 assertion 全部不变,修复前从 4 涨到 8)、`test_first_scope_safety_assessment_and_replay_assess_every_tool_call`(cold start 与 `rebuild_projections()` 之后都必须全量评估)。
- 原始诊断(留档):
