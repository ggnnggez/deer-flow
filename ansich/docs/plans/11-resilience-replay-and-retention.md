...
>
> **实现状态（2026-08-23，P11-C T13 落地）**：本节已实现，带三处记录在案的偏离。
...
> 预算 `ansich.shutdown_budget_ms`（25000，startup-only，本批不再 bump `config_version`）是整段序列的墙：每步取 `min(自己的份额, 剩余)`，提前结束的把剩余让给后面，**某一步超时绝不中止后面的步骤**（否则关停最糟时丢的正是第七步要写下来的证据）。
