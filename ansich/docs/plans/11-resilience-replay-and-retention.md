...
> **实现状态：第 1–5 步已由 P11-C 全部落地**（`deerflow/ansich/replay.py` + `replay_cli.py`，`sql.py::_replay_observation_condition` / `_validate_replace_request` / `mint_replay_jobs` / `read_model_digest`；第 3 步 `--replace` 由 T5 补齐）。本节**九处**必须一起读的偏离与澄清（第 8、9 条由批终的文档收尾补上）：
