...
- Phase 6 已形成本地完整闭环：版本化 assessor/resolver、evidence-watermark job、Alert episode/workflow/read model、wall-clock liveness/dwell 评估、管理员告警 API、幂等 interrupt/rollback 代理、Operations Alerts 页签和 Task behavior Belief 已落地（详见 [06-runaway-alerts-and-intervention.md](06-runaway-alerts-and-intervention.md)）。SQLite 迁移/并发、PostgreSQL DDL 编译、后端 Ansich 回归、前端单测与关键 Playwright 流程已覆盖；真实 PostgreSQL 升级矩阵、关闭 Ansich 的基准和生产 paper drill 仍是最终生产就绪门禁。Phase 7–12 尚未开始实现；页面不得提前显示 release 比较、inclusive usage、授权/副作用或语义评估的占位数据。
...
- Phase 6 代码评审的跟进项登记在 [phase-6-review-followups.md](phase-6-review-followups.md)；其中 L3（wall_time 覆盖语义）建议立即修复，M1（assessor job 合并）与 L1（对账读放大）需在 Phase 7 前完成，L4（未生产的告警类型）需在 Phase 7 前决策归属，L2（孤儿 requested action 回收）留到 Phase 11 前。
