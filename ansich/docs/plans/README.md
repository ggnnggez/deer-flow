...
- Phase 5 代码评审的跟进项登记在 [phase-5-review-followups.md](phase-5-review-followups.md)；M1（assessor 全量 budget 扫描）、M2（heartbeat 断言按秒膨胀）、M3（历史 Task 失去 UI 入口）与 L1（未变 read model 写放大/ETag 失效）已修复，L2（管理员 GET 懒触发 assessment）仍需在 Phase 6 前收窄，L3（usage 贡献契约收紧）留到 Phase 8 前。
